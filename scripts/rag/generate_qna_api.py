...
os.environ["OPENAI_API_KEY"] = (
    open(".env").read().split("OPENAI_API_KEY=")[1].split("\n")[0]
)
...
"""
        ...
        print(f"[{i}/{len(questions)}] ✓ {confidence:.0%}")
        time.sleep(0.5)

    except Exception as e:
        print(f"[{i}/{len(questions)}] ✗ {str(e)[:40]}")
        results.append(
            {
                "Q_No": i,
                "Question": q,
...
print(f"\nSAVED: {filename}")
print(
    f"Total: {len(results)} | Answered: {passed} ({passed / len(results) * 100:.0f}%)"
)
