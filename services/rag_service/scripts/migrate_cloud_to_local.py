...
def migrate():
    ...
    local_client.create_collection(
        collection_name=DEST_COLLECTION, dimension=1536, metric_type="COSINE"
    )
    ...
    while True:
        res = cloud_col.query(
            expr="id >= 0", output_fields=["*"], offset=offset, limit=batch_size, using="cloud"
        )

        if not res:
            break

        local_client.insert(collection_name=DEST_COLLECTION, data=res)
        offset += len(res)
