...
def connect_milvus(alias: str, host: str = None, port: str = None, token: str = None, uri: str = None):
...
def create_target_collection_if_not_exists(alias: str, collection_name: str, source_collection: Collection):
    """Creates the target collection with the same schema as the source if it doesn't exist."""
    if utility.has_collection(collection_name, using=alias):
        logger.info(f"Target collection '{collection_name}' already exists. Dropping to recreate with compatible schema.")
    ...
    for field in source_schema.fields:
        if field.is_primary:
            fields.append(FieldSchema(name=field.name, dtype=field.dtype, is_primary=True, auto_id=False))
        else:
            fields.append(FieldSchema(name=field.name, dtype=field.dtype, max_length=field.max_length, dim=field.dim))
    ...
    if vector_field_name:
        ...
        logger.info(f"Created HNSW index on '{vector_field_name}' for target collection '{collection_name}'.")
...
def migrate_collection():
    if not CLOUD_MILVUS_URI or not CLOUD_MILVUS_TOKEN:
        logger.error("CLOUD_MILVUS_URI and CLOUD_MILVUS_TOKEN environment variables must be set for source connection.")
        return

    logger.info(f"Starting migration of collection '{SOURCE_COLLECTION_NAME}' from Cloud to Local Milvus.")
    ...
    target_collection = create_target_collection_if_not_exists("local_milvus", TARGET_COLLECTION_NAME, source_collection)
    ...
    target_field_names = [f.name for f in target_schema.fields if not f.is_primary and not f.auto_id]
    ...
    for i in range(0, total_entities, BATCH_SIZE):
        ...
        try:
            insert_result = target_collection.insert(insert_lists)
            target_collection.flush()
            migrated_count += len(insert_lists[0]) if insert_lists else 0
            logger.info(f"Migrated {len(insert_lists[0]) if insert_lists else 0} entities. Total migrated: {migrated_count}/{total_entities}.")
        except Exception as e:
    ...
    logger.info(f"Migration completed. Total {migrated_count} entities migrated to local collection '{TARGET_COLLECTION_NAME}'.")
