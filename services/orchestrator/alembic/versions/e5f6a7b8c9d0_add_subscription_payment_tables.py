...
Create subscriptions, payments, and webhook_events tables for Razorpay integration.
...
def upgrade() -> None:
    ...
    subscription_status = sa.Enum(
        'active', 'cancelled', 'expired', 'past_due', 'trialing', 'created',
    ...
    op.create_table(
        ...
        sa.Column('status', subscription_status, nullable=False, server_default='created'),
    ...
    op.create_table(
        ...
        sa.Column('user_id', sa.dialects.postgresql.UUID(as_uuid=True),
                   sa.ForeignKey('users.id', ondelete='CASCADE'), nullable=False),
        sa.Column('subscription_id', sa.dialects.postgresql.UUID(as_uuid=True),
    ...
    # 4. Create webhook_events table (audit log)
    op.create_table(
        'webhook_events',
        sa.Column('id', sa.dialects.postgresql.UUID(as_uuid=True), primary_key=True),
