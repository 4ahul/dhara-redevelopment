...
# 1. Define the job (Runs at 3:00 AM every day)
# Format: minute hour day_of_month month day_of_week command
CLEANUP_JOB="0 3 * * * cd /app && /usr/local/bin/python services/orchestrator/scripts/cleanup_logs.py --days 30 >> /var/log/cleanup.log 2>&1"
