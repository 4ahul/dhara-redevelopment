...
from services.orchestrator.core.dependencies import get_admin_service, require_admin
...
@router.get("/enquiries/{enquiry_id}", response_model=EnquiryResponse, dependencies=[Depends(require_admin)])
...
@router.patch("/enquiries/{enquiry_id}", response_model=EnquiryResponse, dependencies=[Depends(require_admin)])
async def patch_enquiry(enquiry_id: UUID, req: EnquiryUpdate, service: AdminService = Depends(get_admin_service)):
