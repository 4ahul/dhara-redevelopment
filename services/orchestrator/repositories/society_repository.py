...
from models.report import SocietyReport, FeasibilityReport
from models.team import SocietyTender

async def get_society_by_id(db: AsyncSession, society_id: UUID, user_id: UUID) -> Optional[Society]:
...
async def get_feasibility_report(db: AsyncSession, report_id: UUID, user_id: UUID) -> Optional[FeasibilityReport]:
    return (await db.execute(
