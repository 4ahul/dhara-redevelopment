...
async def get_user_by_id(db: AsyncSession, user_id: UUID) -> User | None:
    ...
    return (await db.execute(select(User).where(User.id == user_id, User.is_active))).scalar_one_or_none()

async def get_user_by_email(db: AsyncSession, email: str) -> User | None:
    """Fetch a single user by email."""
    return (await db.execute(select(User).where(User.email == email, User.is_active))).scalar_one_or_none()

async def get_user_by_clerk_id(db: AsyncSession, clerk_id: str) -> User | None:
    """Fetch a single user by their external Clerk ID."""
    return (await db.execute(select(User).where(User.clerk_id == clerk_id, User.is_active))).scalar_one_or_none()
