...
async def list_pmc_users_with_stats(
    ...
    query = select(
        User,
        soc_count_sub.label("societies_count"),
        rep_count_sub.label("reports_count")
...
async def get_dashboard_counts(db: AsyncSession) -> dict[str, int]:
    ...
    stats = {
        "total_pmc_users": (await db.execute(select(func.count(User.id)).where(User.role == UserRole.PMC))).scalar() or 0,
        ...
        "open_enquiries": (await db.execute(select(func.count(Enquiry.id)).where(Enquiry.status.in_([EnquiryStatus.NEW, EnquiryStatus.IN_PROGRESS])))).scalar() or 0,
        "total_get_started": (await db.execute(select(func.count(GetStartedRequest.id)))).scalar() or 0,
