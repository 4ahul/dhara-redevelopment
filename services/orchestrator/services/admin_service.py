...
class AdminService:
    ...
    async def list_pmc_users(self, page: int = 1, page_size: int = 20, search: str = None, is_active: bool = None) -> PaginatedResponse:
    ...
    async def admin_search(self, q: str, entity_type: str = None, page: int = 1, page_size: int = 20) -> PaginatedResponse:
        """
        ...
        return PaginatedResponse(items=results[off:off+page_size], total=total, page=page, page_size=page_size, total_pages=math.ceil(total/page_size) if total else 0)

    async def list_enquiries(self, page: int = 1, page_size: int = 20, status: str = None, source: str = None) -> PaginatedResponse:
        """List enquiries via the Repository layer."""
        items, total = await enquiry_repository.list_enquiries(self.db, page, page_size, status, source)
        serialized = [EnquiryResponse.model_validate(e).model_dump() for e in items]
        return PaginatedResponse(items=serialized, total=total, page=page, page_size=page_size, total_pages=math.ceil(total/page_size) if total else 0)
