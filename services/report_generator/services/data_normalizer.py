...
def _financial_for_scheme(
    ...
    add_fsi_prem_amt = premium_amounts.get(
        "add_fsi_premium", round(add_fsi_sqft / SQFT_PER_SQM * rr_open_land * 0.50)
    )
    fungible_res_amt = premium_amounts.get(
        "fungible_res", round(bua["fungible_total_sqm"] * rr_open_land * 0.35)
    )
    staircase_prem_amt = premium_amounts.get(
        "staircase_prem", round(bua["staircase_sqft"] / SQFT_PER_SQM * rr_open_land * 0.25)
    )
    ...
    fsi_tdr_total = fsi_tdr_line_total or (
        add_fsi_prem_amt
        + fungible_res_amt
        + staircase_prem_amt
        + osd_prem_amt
    ...
    mcgm_total = mcgm_line_total or (
        scrutiny + dev_charges + dev_cess + luc + cfo + heritage + misc
    )
    ...
    # 7. Project total
    project_total = (
        const_with_gst + fsi_tdr_total + mcgm_total + prof_fees + temp_total + stamp_total
    )
...
def normalize_report_data(raw: dict) -> dict:
    ...
    if res_units:
        existing_carpet_sqft = sum(
            float(u.get("total_sqft", u.get("area_sqm", 0) * SQFT_PER_SQM)) for u in res_units
        )
    else:
    ...
    sale_rate = float(flat_fin.get("sale_rate_sqft") or raw.get("sale_rate_sqft") or 65_000)
    ...
    rr_open_land = float(
        rr_section.get("rr_open_land_sqm", 0)
        or raw.get("ready_reckoner", {}).get("rr_open_land_sqm", 200_000)
    )
