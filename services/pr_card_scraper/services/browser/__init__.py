...
class MahabhumiScraper:
    ...
    async def scrape_pr_card(self, **kwargs) -> dict:
        ...
        try:
            ...
            # Returns (image_bytes, image_url) — image_bytes may be populated from base64 decode
            image_bytes, image_url = await active_extractor.wait_for_pr_card_image(timeout=45)

            # If we already have image bytes (e.g., from base64 decode), skip URL navigation
            if not image_bytes:
